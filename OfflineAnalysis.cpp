...
/*
    h.AddChamber("Raman","Ayush","Shachi",
    		     10,40,70,
				 20,50,80,
				 30,60,90,1,
				 200,10,100,
...
/*
    RPC r[2];
    r.SetGap(0, "raman", 10, 20, 30, 1);
    r.SetGap(1, "ayush", 40, 50, 60, 1);
    r.SetGap(2, "shachi", 70, 80, 90, 1);
    r.SetFeb(0,200,300,400,500);
    r.SetFeb(1,10,20,30,40);
    r.SetFeb(2,100,300,500,700);
    r.Print();
*/
...
/*         h.AddChamber(chamber_tn,chamber_tw,chamber_b,
                 		      curr1,curr2,curr3,
         					  vset1,vset2,vset3,
         					  vmon1,vmon2,vmon3,station);
*/
 ...
 */
         ...
         r[i].SetName(chamberName);
         r[i].SetGap(0,chamber_tn, curr1, vset1, vmon1, station);
         r[i].SetGap(1,chamber_tw, curr2, vset2, vmon2, station);
         r[i].SetGap(2,chamber_b, curr3, vset3, vmon3, station);
         r[i].SetFeb(0,feb[1],feb[2],feb[3],feb[4]);
         r[i].SetFeb(1,feb[5],feb[6],feb[7],feb[8]);
         r[i].SetFeb(2,feb[9],feb[10],feb[11],feb[12]);

         //r[i].Print();

         for(int j=0 ; j<r[i].GetGapVectorSize() ; j++)
         {
         Gap temp = r[i].GetGap(j);
         ...
         cout << "FEB values: " ;
         for(int j=0; j<3;j++){
           int numOfChips = r[i].GetFeb(j).GetNumOfChips();
           for(int k=0; k<numOfChips ; k++){
             cout<<r[i].GetFeb(j).GetChip(k).GetThreshold()<<" ";
           }
         }
         ...
         //rpc->Print();
         //std::cout<<rpc->GetGap(0)<<std::endl;
